...
export class AdminMembersDbService {
  ...
  public static async setMemberEditPermission(memberId: string, associationId: string) {
    const { error } = await supabase
      ...
      .update({ edit_association_id: associationId, role: UserRole.EDITOR })
      .eq('id', memberId)
    if (error) {
      console.log(
        'error',
        error,
        error.code,
        PostgrestError.DUPLICATE_KEY,
        error.code == PostgrestError.DUPLICATE_KEY,
      )
  ...
  public static async setMemberCreationPermission(memberId: string) {
    const { error } = await supabase
      .from(TablesList.USER_PROFILES)
      .update({ edit_association_id: null, role: UserRole.CREATOR })
      .eq('id', memberId)
    if (error) {
      addNotification(i18n.t('adminMembers.addPermissionError'), NotificationType.ERROR)
      throw error
  ...
  public static async deleteCreator(memberId: string) {
    const { error } = await supabase.functions.invoke('manage-admin-members', {
      body: {
    ...
    if (error) {
      addNotification(i18n.t('adminMembers.deleteCreatorError'), NotificationType.ERROR)
      throw error
    }
  }
