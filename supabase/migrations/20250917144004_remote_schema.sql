...
-- drop table "storage"."iceberg_tables";
  ...
  create policy "Admins control association logos"
  on "storage"."objects"
  as permissive
  for all
  to public
using (((bucket_id = 'association-logos'::text) AND is_admin()))
with check (((bucket_id = 'association-logos'::text) AND is_admin()));



  create policy "Allow editors to delete logos"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'association-logos'::text) AND (EXISTS ( SELECT 1
   FROM user_profiles
  WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.role = 'editor'::user_roles) OR (user_profiles.role = 'creator'::user_roles)))))));



  create policy "Allow editors to update logos"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
using (((bucket_id = 'association-logos'::text) AND (EXISTS ( SELECT 1
  ...
  create policy "Allow editors to upload logos"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check (((bucket_id = 'association-logos'::text) AND (EXISTS ( SELECT 1
   FROM user_profiles
  WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.role = 'editor'::user_roles) OR (user_profiles.role = 'creator'::user_roles)))))));
  ...
  for select
  to public
using ((bucket_id = 'association-logos'::text));
